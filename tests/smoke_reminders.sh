...
source "$REPO_ROOT/scripts/commands/reminder/_lib/common.sh"
...
done < <(find scripts/commands -type f -name '*.sh' ! -path '*/_lib/*' | sort)

if scripts/commands/account/list.sh --plain >/dev/null 2>&1; then
  printf 'smoke_reminders: --plain should fail\n' >&2
  exit 1
fi

if scripts/commands/reminder/today.sh --format=json >/dev/null 2>&1; then
  printf 'smoke_reminders: --format=json should fail\n' >&2
  exit 1
fi

default_account_json="$(scripts/commands/account/default-account.sh)"
...
default_list_json="$(scripts/commands/account/default-list.sh)"
...
scripts/commands/list/create.sh "$target_list" >/dev/null
scripts/commands/list/create.sh "$filter_list" >/dev/null
scripts/commands/list/create.sh "$extra_list" >/dev/null
wait_for_list "$source_list"
wait_for_list "$target_list"
wait_for_list "$filter_list"

list_exists_json="$(scripts/commands/list/exists.sh "$source_list")"
...
scripts/commands/list/edit.sh "$extra_list" name "${extra_list}_renamed" >/dev/null
edited_exists="$(json_get "$(scripts/commands/list/exists.sh "${extra_list}_renamed")" 'payload.exists')"
...
scripts/commands/reminder/edit.sh --id "$reminder_id" body "Updated body" >/dev/null
edited_body="$(json_get "$(scripts/commands/reminder/get.sh --id "$reminder_id" body)" 'payload.value')"
if [[ "$edited_body" != "Updated body" ]]; then
  printf 'smoke_reminders: reminder edit did not update body: %s\n' "$edited_body" >&2
  exit 1
fi

scripts/commands/reminder/edit-by-id.sh "$wrapper_id" priority medium >/dev/null
...
scripts/commands/reminder/reschedule.sh --id "$wrapper_id" "$rescheduled_due" >/dev/null
wrapper_due_date="$(json_get "$(scripts/commands/reminder/get.sh --id "$wrapper_id" due_date)" 'payload.value')"
...
search_exact_json="$(scripts/commands/reminder/search.sh exact-name "$source_list" "$reminder_name")"
...
scripts/commands/reminder/move-by-id.sh "$wrapper_id" "$target_list" >/dev/null
wrapper_moved_list="$(json_get "$(scripts/commands/reminder/get.sh --id "$wrapper_id" list)" 'payload.value')"
...
scripts/commands/reminder/complete.sh --id "$reminder_id" >/dev/null
completed_state="$(json_get "$(scripts/commands/reminder/get.sh --id "$reminder_id" completed)" 'payload.value')"
if [[ "$completed_state" != "true" ]]; then
  printf 'smoke_reminders: reminder complete did not update state: %s\n' "$completed_state" >&2
  exit 1
fi

exists_json="$(scripts/commands/reminder/exists.sh --id "$reminder_id")"
json_assert "$exists_json" 'payload.exists === true && payload.id === "'"$reminder_id"'"' "reminder exists json is invalid"

delete_json="$(scripts/commands/reminder/delete.sh --id "$reminder_id")"
json_assert "$delete_json" 'payload.deleted === true && payload.id === "'"$reminder_id"'"' "reminder delete json is invalid"

exists_after_delete="$(json_get "$(scripts/commands/reminder/exists.sh --id "$reminder_id")" 'payload.exists')"
if [[ "$exists_after_delete" != "false" ]]; then
  printf 'smoke_reminders: reminder still exists after delete\n' >&2
  exit 1
fi

delete_by_id_json="$(scripts/commands/reminder/delete-by-id.sh "$wrapper_id")"
json_assert "$delete_by_id_json" 'payload.deleted === true && payload.id === "'"$wrapper_id"'"' "reminder delete-by-id json is invalid"

wrapper_exists_after_delete="$(json_get "$(scripts/commands/reminder/exists.sh --id "$wrapper_id")" 'payload.exists')"
...
today_json="$(scripts/commands/reminder/create.sh "$filter_list" "$today_reminder" --due "$today_due" --priority high)"
today_id="$(json_get "$today_json" 'payload.id')"

today_completed_json="$(scripts/commands/reminder/create.sh "$filter_list" "$today_completed_reminder" --due "$today_due")"
today_completed_id="$(json_get "$today_completed_json" 'payload.id')"
scripts/commands/reminder/complete.sh --id "$today_completed_id" >/dev/null

overdue_json="$(scripts/commands/reminder/create.sh "$filter_list" "$overdue_reminder" --due "$overdue_due")"
overdue_id="$(json_get "$overdue_json" 'payload.id')"

upcoming_json="$(scripts/commands/reminder/create.sh "$filter_list" "$upcoming_reminder" --due "$upcoming_due" --priority medium)"
upcoming_id="$(json_get "$upcoming_json" 'payload.id')"

future_json="$(scripts/commands/reminder/create.sh "$filter_list" "$future_reminder" --due "$future_due")"
future_id="$(json_get "$future_json" 'payload.id')"

tomorrow_early_json="$(scripts/commands/reminder/create.sh "$filter_list" "$tomorrow_early_reminder" --due "$tomorrow_early_due")"
tomorrow_early_id="$(json_get "$tomorrow_early_json" 'payload.id')"

today_filter_json="$(scripts/commands/reminder/today.sh "$filter_list")"
...
overdue_filter_json="$(scripts/commands/reminder/overdue.sh "$filter_list")"
json_assert "$overdue_filter_json" 'payload.length === 1 && payload[0].id === "'"$overdue_id"'"' "overdue filter is unexpected"

upcoming_filter_json="$(scripts/commands/reminder/upcoming.sh 3 "$filter_list")"
json_assert "$upcoming_filter_json" 'payload.map(item => item.id).sort().join(",") === ["'"$today_id"'","'"$tomorrow_early_id"'","'"$upcoming_id"'"].sort().join(",")' "upcoming filter is unexpected"

due_before_json="$(scripts/commands/reminder/due-before.sh "$today_start" "$filter_list")"
json_assert "$due_before_json" 'payload.length === 1 && payload[0].id === "'"$overdue_id"'"' "due-before filter is unexpected"

due_range_json="$(scripts/commands/reminder/due-range.sh "$range_start" "$range_end" "$filter_list")"
json_assert "$due_range_json" 'payload.length === 1 && payload[0].id === "'"$upcoming_id"'"' "due-range filter is unexpected"

focus_json="$(scripts/commands/reminder/today-or-overdue.sh "$filter_list")"
...
search_text_json="$(scripts/commands/reminder/search.sh text "$future_reminder" "$filter_list")"
